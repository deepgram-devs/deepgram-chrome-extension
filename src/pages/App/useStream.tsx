  ...
  const handleStream = (tokenRef, resultRef, setTranscript) => {
    return async () => {
      ...
      const livestreamOptions = deepgramOptions
        ? deepgramOptions.livestreamOptions
        : {};
      ...
      if (!token) {
        ...
        try {
          ...
          socketRef.current.addEventListener('error', (err) => {
            setIsStreaming(false);
            alert(
              'Failed to establish connection. Please make sure you have enough credit in your project.'
            );
            if (socketRef.current) socketRef.current.close();
            if (recorderRef.current) recorderRef.current.stop();
            if (screenStream) {
        ...
        recorderRef.current = new MediaRecorder(mixed, {
          mimeType: 'audio/webm',
        });
